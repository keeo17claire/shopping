...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $newAmount = $_POST['Increase_Amount'];
  ...
  // Insert a new record into the added_ingredient table to save the history
  $insertSQL = sprintf(
    "INSERT INTO added_ingredient (UserID, AddCode, IngID, Date, NumAdded, OriAmount) VALUES (%s, %s, %s, NOW(), %s, %s)",
    GetSQLValueString($userID, "int"), // replaced $_POST['UserID'] with $userID
    GetSQLValueString($addCode, "int"), // replaced $_POST['AddCode'] with $addCode
    GetSQLValueString($_POST['IngID'], "int"),
    GetSQLValueString($newAmount, "int"),
    GetSQLValueString($originalAmount, "int")
  );
...
<body>
  ...
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
    <table align="center">
      ...
      <tr valign="baseline">
        <td nowrap="nowrap" align="right">จำนวนที่ต้องการเพิ่ม:</td>
        <td><input type="text" name="Increase_Amount" value="" size="32" /></td>
      </tr>
